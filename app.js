...
- to view report        : report`
  ...
  switch (temp[0].toUpperCase()) {
    ...
    case 'IN':
      ...
      if(amount < 0) amount = amount*-1.00
      status = `RM ${parseFloat(amount).toFixed(2)} recorded in ${acc} account`
    } else {
      await Account.update({verb: temp[0].toUpperCase(), amount:amount*1},{where:{message_id:r.message_id}})
      if(amount < 0) amount = amount*-1.00
      status = `Record updated. New amount is RM ${parseFloat(amount).toFixed(2)}`
      }
      break
    case 'REPORT':
